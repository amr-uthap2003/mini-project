...
// Now create query based on the actual columns
...
// Add session_type only if it exists
...
$sessions_query .= " FROM tb_bookings b
    ...
    WHERE b.counselor_id = ? AND b.status = 'approved'
...
// Store column existence for later use in template
...
<body class="bg-light">
    ...
    <div class="container py-4">
        <ul class="nav nav-tabs">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="class_management.php">
                    <i class="bi bi-calendar-check"></i> Sessions
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="counselor_past_sessions.php">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-white">
                        <h5 class="mb-0">Approved Sessions</h5>
                    </div>
                    <div class="card-body">
                                    ...
                                    <tbody>
                                        <?php while ($session = $sessions_result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ($has_session_type): ?>
                                                        <?php if ($session['session_type'] === 'video'): ?>
                                                            <a href="video_call.php?booking_id=<?php echo $session['booking_id']; ?>" 
                                                               class="btn btn-sm btn-primary">
                                                                <i class="bi bi-camera-video"></i> Join Meeting
                                                            </a>
                                                        <?php elseif ($session['session_type'] === 'text'): ?>
                                                            <a href="chat.php?booking_id=<?php echo $session['booking_id']; ?>" 
                                                               class="btn btn-sm btn-success">
                                                                <i class="bi bi-chat-dots"></i> Start Chat
                                                            </a>
                                                        <?php endif; ?>
                                                        <a href="update_session.php?booking_id=<?php echo $session['booking_id']; ?>" 
                                                           class="btn btn-sm btn-outline-secondary">
                                                            <i class="bi bi-gear"></i> Change Type
                                                        </a>
                                                    <?php else: ?>
                                                        <!-- Default action buttons when session_type is not available -->
                                                        <a href="chat.php?booking_id=<?php echo $session['booking_id']; ?>" 
                                                           class="btn btn-sm btn-success">
                                                            <i class="bi bi-chat-dots"></i> Start Chat
                                                        </a>
                        ...
                        <?php else: ?>
                            <div class="text-center py-4">
                                <p class="text-muted">No approved sessions found.</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
